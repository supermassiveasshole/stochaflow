...
若未来 latent 方法需要新的 SNR weighting，必须在该任务实现时经独立 roadmap
决策、recipe、正式 Evaluation protocol 与 task-owned acceptance 决策同步引入；已退休的
pixel-space 实验不提供可复用 Builder、参数、codec/Process 配置或 checkpoint authority。
...
状态（2026-07-30）：embedded-state 基础设施已通过一个独立 extension contract fixture
的辅助 inference asset vertical slice 闭合。该验收覆盖 fresh train、
