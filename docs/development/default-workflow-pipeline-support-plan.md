...
8. **训练完成后的正式评估是独立 Evaluation operation。**
   ...
   data split、sample plan、protocol、result 和 gate；训练期 validation、
   periodic diagnostic、final sampling 都不能替代它。
...
orchestrator；项目脚本或外部系统可以调用后续的库级 `run_training()` /
`run_sampling()`。
...
### 7.3 训练后的 final sample

当前 training runner 可以在 best checkpoint 后硬编码触发一次 sampling。重构后它应是
明确的 `TrainingRunOptions.run_final_sampling` 政策，并通过公共 sampling operation
调用，不复制 sampling 逻辑。

对于需要外部输入的 SR recipe，默认 training config 应把 final sampling 关闭；用户
随后用 restore/sample entry 显式提供 LR input。训练 runner 不应从 validation batch
猜一个 condition 来生成 artifact。
...
```text
teacher TrainingRunOutcome.best_checkpoint
-> explicit teacher bundle exporter
-> teacher bundle
-> distillation TrainingRunRequest
-> student TrainingRunOutcome.best_checkpoint
...
```python
type WorkflowStage = TrainingStage | SamplingStage | ExportStage
...
在修改[Consistency 计划](consistency-distillation-support-plan.md)前：

- `online_stopgrad` 只能命名为 experimental target policy；
- 不得以其结果代表标准 consistency distillation；
- Stage 3/5 的 promotion gate 应改为先完成 target lifecycle；
- config 默认应在实现后改为 `ema_target`；
...
### Stage W0：是否需要 Workflow orchestrator 的决策 gate
...
- incomplete/non-finite gate fail；
