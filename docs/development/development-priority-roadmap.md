...
当前不应平均推进所有 proposal。唯一主关键路径是：

```text
关闭当前基线
  -> Train/Sample authority cutover
  -> inference asset projection
...
- Hydra H0–H3、Metrics/Evaluation 和 example cleanup 不阻塞首个 AFHQ latent
  correctness 闭环；它们在闭环后、配置和正式训练规模扩大前完成。
...
截至 2026-07-29，built-in authoring surface 已先完成一项可独立关闭的整理：MNIST
...
```mermaid
flowchart LR
    B0["B0 基线关闭"] --> C1["C0/C1 Train/Sample authority"]
    C1 --> A1["LD1 inference asset projection"]
    C1 --> A2["LD2 pretrained AutoencoderKL"]
    A1 --> A3["LD3 AFHQ vertical slice"]
    A2 --> A3

    A3 --> P1["LD4A prepared posterior"]
    A3 --> P2["LD4B optimizer-step production loop"]
    A3 --> P3["LD4C codec asset bundle"]

    A3 --> H["Hydra H0-H3 + retained example cleanup"]
    A3 --> M["Metrics M0-M1"]
    M --> E["Evaluation E0-E1 + latent quality profile"]
...
`LD1` 与 `LD2` 可以由不同开发者并行；单人开发时仍先冻结 inference asset contract，
再完成 provider 接线。`Metrics` 与 `Evaluation` 不阻塞 AFHQ smoke，但其正式结果
contract 阻塞公开质量结论。
...
### M0 — Pretrained Codec Ready（P1，1–2 个工程周）

合并两条可并行工作：

1. Latent Phase 1：闭合
   `TrainingPlan -> CheckpointManager -> SamplingCheckpointView ->
   InferenceAssetProvider -> SamplingBuilderContext`；
2. Latent Phase 2：实现 optional Diffusers `AutoencoderKL` codec provider。
...
### M1 — AFHQ Latent Diffusion Vertical Slice（P1，4–7 个工程日）
...
### M2 — Production Latent Training Substrate（P1/P2，1–2 个工程周）
...
Phase A embedded codec 只允许用于 M0/M1 correctness，不用于正式长训练。

### Q0 — Config and Quality Foundation（P2，1–2 个工程周，可并行）
...
- Metrics M0–M1：统一 naming、validation `MetricEngine` 和 canonical epoch result；
- Evaluation E0–E1，并补充 latent reconstruction 与 decoded-generation profile 的
  必要子集；
- Evaluation 复用 M0 的 inference projection，不再实现第二套 checkpoint subject
  asset resolver。

Hydra H4 multirun、完整 prediction artifact suite 和完整 comparison/gate 均不属于
Q0。

### M3 — Open-data DiT Baseline（P2，1–2 个工程周 + 实验时间）
...
### M4 — Stable Diffusion 1.x Native Sampling（P3，2–3 个工程周）
...
### M5 — Stable Diffusion 1.x Full Fine-tuning（P3，2–4 个工程周 + 实验时间）
...
### M6 — Scale and New Families（P4/Later，decision-gated）
...
| [Hydra Configuration Migration](hydra-configuration-composition-migration-plan.md) | Split | P0 + P2 + Later | C0/C1 -> M1 -> H0-H3 | C0/C1 先行；H0-H3 在 M1 后；H4 延期 |
| [Latent Diffusion](latent-diffusion-support-plan.md) | Current mainline | P1/P2 | B1 -> M0–M3 | 当前主要开发和实验资源 |
| [Metrics](metrics-support-plan.md) | Parallel enabler | P2 | M1 后，M3 前 | 先 M0–M1；M2–M4 按正式监控需求 |
| [Post-training Evaluation](post-training-evaluation-support-plan.md) | Formal-release gate | P2 | Metrics M0–M1 + shared inference projection | 先 E0–E1 与 latent profile；其余渐进 |
| [Stable Diffusion Component-Native](stable-diffusion-component-native-support-plan.md) | Next product line | P3 | M2/M3 共享前置稳定 | SD1–SD6；SD7–SD8 后置 |
...
| [Distributed Training and Inference](distributed-training-and-inference-support-plan.md) | Conditional | P4 | M3/M5 profiling 证明需要 | 先 D0 profiling；再决定 DDP/FSDP2 |
...
- M1 可声明 experimental functional support，不声明规模或质量。
- M3 前必须有结构化 reconstruction gate 和 decoded-generation evaluation。
...
2. `Separate train and sample authority`
3. `Project inference assets through checkpoints`
4. `Add pretrained AutoencoderKL codec`
5. `Add AFHQ latent diffusion vertical slice`
6. `Add prepared posterior artifacts`
7. `Add optimizer-step production lifecycle`
8. `Deduplicate codec assets in run bundles`
9. `Add metrics and checkpoint evaluation foundation`
10. `Adopt Hydra for retained fresh-train configs`
11. `Add The Met DiT baseline`
12. `Add Stable Diffusion native sampling`
13. `Add Stable Diffusion full fine-tuning`
...
- M0 表明现有 checkpoint projection 无法支持 self-contained codec recovery；
- M1 表明 image-backed encode 成为 correctness 测试本身的主要瓶颈；
- M2 表明 asset bundle 或 step-based loop 需要新的训练 loop family；
