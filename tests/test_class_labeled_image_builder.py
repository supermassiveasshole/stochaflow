...
def _identity(root: Path, *, source_name: str) -> ManagedDataArtifactIdentity:
...
class IndependentClassLabeledImageSource(ImageDataSource):
    ...
    def materialize(
        ...
        validation = (train[0], train[3]) if self.params.get(
            "native_validation", False
        ) else None
        return ManagedDataArtifact(
            artifact_root=root,
            manifest_path=root / "manifest.json",
            identity=_identity(root, source_name=_SOURCE_NAME),
...
class IndependentPlainImageSource(ImageDataSource):
    ...
    def materialize(
        ...
        return ManagedDataArtifact(
            artifact_root=root,
            manifest_path=root / "manifest.json",
            identity=_identity(root, source_name=_PLAIN_SOURCE_NAME),
