...
class Cifar10DataSplits:
    """Train/validation/test dataloaders for the CIFAR-10 DDPM script."""
...
def _build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/ddpm_cifar10.yaml"),
        help="Path to the CIFAR-10 DDPM config file.",
    )
    parser.add_argument(
        "--epochs",
        type=int,
...
def _dataset_length(dataset: Dataset) -> int:
    ...
    if not isinstance(dataset, Sized):
        raise TypeError("CIFAR-10 train/validation splitting requires a sized dataset")
    return len(dataset)


def _training_dataset_config(config):
    dataset_config = deepcopy(config.data.dataset)
    dataset_config.params["split"] = "train"
    return dataset_config


def _validation_dataset_config(config):
    dataset_config = deepcopy(config.data.dataset)
    dataset_config.params["split"] = "train"
    dataset_config.params["random_horizontal_flip"] = False
    return dataset_config


def _build_cifar10_data_splits(config) -> Cifar10DataSplits:
    """Build CIFAR-10 train/validation subsets and official test split."""

    full_train_dataset = build_dataset(_training_dataset_config(config))
    full_valid_dataset = build_dataset(_validation_dataset_config(config))
    ...
    indices = torch.randperm(full_train_size, generator=split_generator).tolist()
...
def _cifar10_sample_shape(
    config_channels: int, image_size: int, num_samples: int
) -> torch.Size:
    """Build the full batch-first CIFAR-10 sample shape."""
...
def _dump_sampling_artifacts(
    ...
    *,
    ...
    """Generate CIFAR-10 DDPM samples and reverse-trajectory artifacts."""
    ...
    if not isinstance(training.diffusion, DDPM):
        raise TypeError("train_cifar10_ddpm.py expects the built diffusion to be DDPM")
...
def main() -> None:
    ...
    if config.data.dataset.name != "cifar10":
        raise ValueError(
            "train_cifar10_ddpm.py expects a CIFAR-10 config, got "
            f"'{config.data.dataset.name}'"
        )
    if config.diffusion.name != "ddpm":
        raise ValueError(
            f"train_cifar10_ddpm.py expects a DDPM config, got '{config.diffusion.name}'"
        )
    ...
    data = _build_cifar10_data_splits(config)
    ...
    try:
        ...
        if not args.skip_sampling:
            ...
            sample_shape = _cifar10_sample_shape(channels, image_size, args.num_samples)
